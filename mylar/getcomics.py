...
class GC(object):
    ...
    def parse_downloadresults(self, id, mainlink, comicinfo=None, packinfo=None, link_type_failure=None):
        ...
        # Second fallback: always scan for <a href*="/dlds/"> with title/text "Download Now" or "Mirror Download"
        # and add them if not already in valid_links (so GC-Main/Mirror are found even when other links were parsed first)
        existing_hrefs = set()
        for k, y in valid_links.items():
            if 'links' in y:
                for a in y['links']:
                    existing_hrefs.add(a.get('links'))
        logger.info('[DDL-GATHERER-OF-LINKAGE] Scanning page for /dlds/ main and mirror links...')
                        ...
                        for key_option in ['HD-Upscaled:download now', 'HD-Upscaled:main', 'HD-Upscaled:mirror download',
                            ...
                            if key_option in site_position:
                                try:
                                    ...
                                    if pos < len(tmp_links):
                                        kk = tmp_links[pos]
                                        logger.info('[MAIN-SERVER] Found download option: %s - attempting %s' % (key_option, kk['series']))
                                        link_matched = True
                                        found_link = True
                                        break
                                    else:
                                        logger.fdebug('Position %s for %s is out of range (tmp_links has %s items)' % (pos, key_option, len(tmp_links)))
                                except (KeyError, IndexError) as e:
                                    logger.fdebug('Error accessing link for %s: %s' % (key_option, e))
                                    continue
                        
                        if not found_link:
                            logger.warn('[MAIN-SERVER] No suitable download option found in site_position. Available options: %s' % list(site_position.keys()))
        ...
        if link is None:
            logger.warn('[PARSE-DOWNLOAD-RESULTS] No valid download link found after processing all options. Available options were: %s' % list(site_position.keys()) if 'site_position' in locals() else 'unknown')
            return {'success': False, 'links_exhausted': link_type_failure}
